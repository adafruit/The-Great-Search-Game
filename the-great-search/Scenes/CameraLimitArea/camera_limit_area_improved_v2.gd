...
@export var force_camera_update: bool = true  # Force camera room update on exit
...
@export var modify_room_position: bool = false
@export var custom_room_offset: Vector2 = Vector2.ZERO
...
var affected_rooms: Array[Vector2] = []
...
func store_original_camera_settings() -> void:
	if camera:
		...
		elif camera.has_method("is_following_player"):
	...
	for child in get_children():
		if child is CollisionShape2D and child.shape:
			if child.shape is RectangleShape2D:
				...
				if "room_size" in camera:
					var room_size = camera.room_size
					var top_left_room = Vector2(
						floor(left_limit / room_size.x),
						floor(top_limit / room_size.y)
					)
					var bottom_right_room = Vector2(
						floor(right_limit / room_size.x),
						floor(bottom_limit / room_size.y)
					)
					
					# Add all rooms in the area to affected_rooms
					for x in range(top_left_room.x, bottom_right_room.x + 1):
						for y in range(top_left_room.y, bottom_right_room.y + 1):
							affected_rooms.append(Vector2(x, y))
...
func _on_body_entered(body: Node2D) -> void:
	if body.is_in_group("player") and camera and !is_active:
		...
		# Apply custom room positions if enabled
		if modify_room_position and "room_positions" in camera:
			for room in affected_rooms:
				var room_key = str(room.x) + "," + str(room.y)
				var room_center = (room * camera.room_size) + (camera.room_size / 2)
				
				# Set custom position for this room
				if camera.has_method("set_room_position"):
					camera.set_room_position(room.x, room.y, room_center + custom_room_offset)
		
		# Apply camera mode override if needed
		if override_camera_follow:
			if "follow_player" in camera:
				camera.follow_player = camera_follow_mode
			elif camera.has_method("toggle_follow_mode"):
				if camera_follow_mode != original_follow_mode:
...
func _on_body_exited(body: Node2D) -> void:
	if body.is_in_group("player") and body == player and camera and is_active:
		...
		# Force camera to update room position
		if force_camera_update and camera.has_method("force_update"):
			camera.force_update()
...
func _draw() -> void:
	if show_debug:
		...
		if camera:
			...
			# Draw affected rooms if available
			if "room_size" in camera and affected_rooms.size() > 0:
				var room_size = camera.room_size
				for room in affected_rooms:
					var room_rect = Rect2(
						(room.x * room_size.x) - global_position.x,
						(room.y * room_size.y) - global_position.y,
						room_size.x,
						room_size.y
					)
					draw_rect(room_rect, Color(0, 0, 1, 0.1), false, 1.0)
