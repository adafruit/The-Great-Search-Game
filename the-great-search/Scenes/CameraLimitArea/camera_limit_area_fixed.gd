...
func _setup_camera() -> void:
	...
	# If not found, look for any Camera2D among viewport children
	if !camera:
		var viewport_children = get_viewport().get_children()
		...
		# If still not found, try to find any Camera2D in the scene
		if !camera:
			var all_cameras = get_tree().get_nodes_in_group("camera")
			if !all_cameras.is_empty():
				for potential_camera in all_cameras:
					if potential_camera is Camera2D:
...
func _on_body_entered(body: Node2D) -> void:
	if body.is_in_group("player") and camera and !is_active:
		...
		elif override_camera_follow and "follow_player" in camera:
