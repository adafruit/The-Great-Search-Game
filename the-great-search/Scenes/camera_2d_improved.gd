...
@export var remember_room_positions := true  # Store camera positions for each room
...
var initial_offset := Vector2.ZERO  # Store the initial offset for consistency
...
func _ready() -> void:
	...
	if !players.is_empty():
		...
		initial_offset = global_position - (player.global_position.floor() / room_size).floor() * room_size - (room_size / 2)
...
func _on_free_roam_area_body_entered(body) -> void:
	...
	if body.is_in_group("player"):
		...
		# Store the current position for the current room before switching to follow mode
		if remember_room_positions and !follow_player:
			var room_key = str(current_room.x) + "," + str(current_room.y)
			room_positions[room_key] = global_position
...
func _on_free_roam_area_body_exited(body) -> void:
	...
	if body.is_in_group("player"):
		...
		if !follow_player:
			...
			if remember_room_positions and room_key in room_positions:
				target_position = room_positions[room_key]
			else:
				target_position = current_room * room_size + (room_size / 2) + position_adjustment + initial_offset
...
func _process(delta: float) -> void:
	...
	if in_free_roam_area or follow_player:
		...
		var room_key = str(target_room.x) + "," + str(target_room.y)
		
		if target_room != current_room:
			current_room = target_room
			
			# If we want to remember room positions and we've been in this room before
			if remember_room_positions and room_key in room_positions:
				# Use the stored position for this room
				target_position = room_positions[room_key]
			else:
				# Calculate the default position using the room center plus both adjustments
				target_position = current_room * room_size + (room_size / 2) + position_adjustment + initial_offset
...
func toggle_camera_mode() -> void:
	# Store the current position for the current room before toggling
	if remember_room_positions and !follow_player:
		var room_key = str(current_room.x) + "," + str(current_room.y)
		room_positions[room_key] = global_position
...
func set_position_adjustment(new_adjustment: Vector2) -> void:
	# Store the current position for the current room before adjustment
	if remember_room_positions and !follow_player and !in_free_roam_area:
		var room_key = str(current_room.x) + "," + str(current_room.y)
		room_positions[room_key] = global_position
	...
	if !in_free_roam_area and !follow_player:
		...
		# After changing position_adjustment, recalculate and store for this room
		target_position = current_room * room_size + (room_size / 2) + position_adjustment + initial_offset
...
# Reset stored room positions (can be called from other scripts)
func reset_room_memory() -> void:
